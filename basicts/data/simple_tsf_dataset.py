...
class TimeSeriesForecastingDataset(BaseDataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        index += self.T_long - self.input_len
        start_index = index + self.input_len - self.T_long

        if self.mode == 'train' and start_index < 0:
            long_history_data = np.zeros((self.T_long, data.shape[1], data.shape[2]), dtype=np.float32)
        else:
            long_history_data = data[start_index:index + self.input_len]
